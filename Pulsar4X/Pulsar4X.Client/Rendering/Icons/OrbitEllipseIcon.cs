...
*/
        ...
        public override void OnFrameUpdate(Camera2 camera)
        {
            ...
            if(DrawShapes == null) DrawShapes = new Shape[1];
            DrawShapes[0].Points = new Vector2[_numberOfDrawSegments + 1];
            DrawShapes[0].Color = new SDL.SDL_Color() { r = _userSettings.Red, g = _userSettings.Grn, b = _userSettings.Blu, a = 255 };

            int index = _index;

            Vector2 parentOffset = _orbitDB.Parent == null ? new Vector2() : camera.ScaledPosition(_orbitDB.Parent.GetDataBlob<PositionDB>().AbsolutePosition);

            var scaledOrigin = camera.ScaledPosition(new Vector3() { X = _bodyAbsolutePos.X, Y = _bodyAbsolutePos.Y, Z = 0 });
            DrawShapes[0].Points[0] = new Vector2() { X = scaledOrigin.X, Y = scaledOrigin.Y };
            for(int i = 1; i < _numberOfDrawSegments + 1; i++)
            {
                ...
                var scaledPosition = camera.ScaledPosition(new Vector3() { X = _points[index].X, Y = _points[index].Y, Z = 0 });
                scaledPosition += parentOffset;
                DrawShapes[0].Points[i] = new Vector2() { X = scaledPosition.X, Y = scaledPosition.Y };
            }
        }
        ...
        public override void OnFrameUpdate(Matrix matrix, Camera camera)
        {
            ...
            var foo = camera.ScaledPosition(WorldPosition_m); //camera position and zoom
            ...
            // Changes below to use DrawShapes for the new renderer
            Scale = 1f;
