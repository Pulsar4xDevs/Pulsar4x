...
namespace Pulsar4X.SDL2UI
{
    public class ResearchWindow : PulsarGuiWindow
    {
        ...
        private Dictionary<Guid, TechSD> _researchableTechsByGuid;
        ...
        private void OnFactionChange()
        {
            ...
            Dictionary<Guid,TechSD> byGuid = new Dictionary<Guid, TechSD>();
            foreach (var item in _factionTechDB.GetResearchableTechs())
            {
                int max = ResearchProcessor.CostFormula(_factionTechDB, item.tech);
                researchableTechs.Add((item.tech, item.pointsResearched, max));
                byGuid.Add(item.tech.ID, item.tech);
            }
        ...
        internal override void Display()
        {
            if (IsActive && ImGui.Begin("Research and Development", ref IsActive, _flags))
            {
                ...
                ImGui.BeginChild("Researchables");
                for (int i = 0; i < _researchableTechs.Count; i++)
                {
                    if (_researchableTechs[i].amountMax > 0) //could happen if bad json data?
                    {
                        ...
                        if (_researchableTechs[i].amountDone > 0) ;
                        float frac = _researchableTechs[i].amountDone / _researchableTechs[i].amountMax;
                        ImGui.ProgressBar(frac, new Vector2(150, 2));
                    }
                }
                ImGui.EndChild();
                ...
                for (int i = 0; i < _scienceTeams.Count; i++)
                {
                    ...
                    var scint = _scienceTeams[i].scientist;
                    ...
                    ImGui.NextColumn();
                    if (scint.ProjectQueue.Count > 0)
                    {

                        TechSD proj = _researchableTechsByGuid[scint.ProjectQueue[0]];
                        ...
                        {
                            string queue = "";
                            foreach (var item in _scienceTeams[i].scientist.ProjectQueue)
                            {
                                queue += _researchableTechsByGuid[item].Name + "\n";
                            }
        ...
        private void SelectedSci(int selected)
        {
            ...
            {
                var proj = _state.Game.StaticData.Techs[_scienceTeams[selected].scientist.ProjectQueue[0]];
                ImGui.Text(proj.Name);
